...
final class ExecutorOrderShowHistoryGatewayTest: XCTestCase {
    func test_fetchTariff() async {
        ...
        let gateway = ExecutorOrderShowHistoryGateway()
        let result = try? await gateway.fetchOrderHistoryShow(orderId: 1793037)
